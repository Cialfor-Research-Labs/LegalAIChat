...
import type { HeaderStatusSummary } from './Layout';
...
interface LegalChatProps {
    ...
    onHeaderStatusChange?: (summary: HeaderStatusSummary | null) => void;
}
...
export const LegalChat = ({
    ...
    const headerStatusSummary = useMemo<HeaderStatusSummary>(
        () => ({
            label: progressLabel,
            progressValue,
            completedSteps,
            helperText: 'We only ask follow-up questions when your first description still leaves important gaps.',
        }),
        [completedSteps, progressLabel, progressValue],
    );
    const IntakeToggleIcon = isIntakeExpanded ? ChevronUp : ChevronDown;

    useEffect(() => {
        onHeaderStatusChange?.(headerStatusSummary);
        return () => onHeaderStatusChange?.(null);
    }, [headerStatusSummary, onHeaderStatusChange]);
